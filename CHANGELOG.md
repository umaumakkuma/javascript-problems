...
### 検討中のアイデア（未決定）
- **段階的ヒントの仕組み** — 各カテゴリに `hints.md`（第1ヒント: jsprimerの節へのピンポイント参照、第2ヒント: 使うメソッド名まで。解答は書かない）。ヒントをどこまで許すかの方針判断待ち
- **学習ログのテンプレート** — `LEARNING_LOG.md` にカテゴリごとの振り返り（意外だった挙動・人に説明するなら・腑に落ちていないこと）を記録する形式。振り返りを必須にするかの方針判断待ち
- 提案済みだが現時点では見送り: AIコードレビュー型問題のカテゴリ新設、メンター向け採点ガイド（模範解答の別管理）
