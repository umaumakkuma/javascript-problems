...
- **template-integrity** — 本体リポジトリ専用（`if: github.repository == ...`でフォークでは走らない）。Node 20/22/24のマトリクスで「未回答テンプレートがクラッシュしない」「`check.js`の出力が`回答済み: 0/`である（=解答がコミットされていない）」を検証する。**解答入りのコミットや、未回答でクラッシュする問題を追加するとCIが落ちる**

学習者向けジョブのNodeは22固定、20以上のサポート保証はtemplate-integrityのマトリクスが担う。
