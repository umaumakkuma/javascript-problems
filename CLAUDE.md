...
pushとPRで自動実行される。3ジョブ構成:

- **basic-check / advanced-test** — 学習者向けフィードバック（フォークでも動く）。未回答は失敗にせず、回答のクラッシュのみ失敗。結果は`GITHUB_STEP_SUMMARY`に日本語で出力
- **template-integrity** — 本体リポジトリ専用（`if: github.repository == ...`でフォークでは走らない）。Node 22/24のマトリクスで「未回答テンプレートがクラッシュしない」「`check.js`の出力が`回答済み: 0/`である（=解答がコミットされていない）」を検証する。**解答入りのコミットや、未回答でクラッシュする問題を追加するとCIが落ちる**

サポート下限はNode.js 22（20は2026年4月にEOL）。学習者向けジョブは採点環境として**下限の22に固定**する——これにより「新しいNodeでしか動かない回答」をCIが検出できる。下限を変えるときはREADMEの必要環境・採点環境の記述とマトリクスを揃えて更新すること。
